...
from agent_server.app.agent.deep_research.state.state_research import ResearcherState, ResearcherOutputState
from agent_server.app.agent.deep_research.research_tool import search_tool, think_tool
from agent_server.app.agent.deep_research.prompt.prompts_zh import (
    research_agent_prompt, 
    compress_research_system_prompt, 
    compress_research_human_message
)

logger = build_logger("research-agent")
...
model = ModelFactory.get_model("deepseek", "deepseek-chat")
model_with_tools = model.bind_tools(tools)
summarization_model = ModelFactory.get_model("deepseek", "deepseek-chat")
compress_model = ModelFactory.get_model("deepseek", "deepseek-chat")
...
def compress_research(state: ResearcherState) -> dict:
    ...
    logger.debug("compress_research",messages)
...
path = os.path.join(Settings.basic_settings.TEMP_IMAGE_PATH, "researcher_agent.png")
...
if __name__ == "__main__":    
    ...
    #Graph节点循环调用，已达到最大步骤数，且未触发停止条件会报错，因此设置recursion_limit最大递归限制
    result = researcher_agent.invoke({"researcher_messages": [HumanMessage(content=f"{research_brief}.")]}, {"recursion_limit": 100})
    print(result)
