...
# 加载参数配置
host = Settings.db_settings.MYSQL_HOST
port = Settings.db_settings.MYSQL_PORT
user = Settings.db_settings.MYSQL_USER
mysql_pw = Settings.db_settings.MYSQL_PASSWORD
db = Settings.db_settings.MYSQL_DATABASE
...
def fig_inter(py_code: str, fname: str) -> str:
    ...
    base_dir = Settings.basic_settings.DATA_PATH
...
# 聊天模型
model_provider = Settings.model_settings.DEFAULT_LLM_PLATFORM
model_name = Settings.model_settings.DEFAULT_LLM_MODEL
model = ModelFactory.get_model(model_provider, model_name, False, [])
